...
_RUN_NAME = "ops-content"
_WIN_RUN_KEY = r"Software\Microsoft\Windows\CurrentVersion\Run"


def launched_from_autostart(argv: list[str] | None = None) -> bool:
    if os.environ.get("OPS_CONTENT_AUTOSTART", "").strip().lower() in {
        "1",
        "true",
        "yes",
    }:
...
def _write_win_launcher() -> Path:
    ...
    path.write_text(
        ...
        f'set OPS_CONTENT_DATA={data}\r\n'
        f'set OPS_CONTENT_AUTOSTART=1\r\n'
...
def _linux_desktop() -> Path:
    ...
    return config / "autostart" / "ops-content.desktop"
