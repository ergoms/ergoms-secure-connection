...
def render_include(
    *,
    ...
    lines = [
        "# Generated by python -m desktop ssh-setup. Do not edit; rerun the command.",
        "# On a new PC: same config.json + same key, then ssh-setup again.",
        "",
    ]
    for raw in client_ssh_block(cfg)["hosts"]:
        ...
        ident_line = posix_path(Path(host_ident).expanduser()) if host_ident else ident
...
def ensure_user_include(ssh_dir: Path) -> Path:
    config = ssh_dir / "config"
    ssh_dir.mkdir(parents=True, exist_ok=True)
    existing = ""
    if config.is_file():
        existing = config.read_text(encoding="utf-8")
    body = _strip_managed_hosts(_strip_include_block(existing)).rstrip()
    block = "\n".join(
        [
            INCLUDE_BEGIN,
            f"Include {INCLUDE_NAME}",
...
def write_setup(
    ...
    *,
    ...
    include = ssh_dir / INCLUDE_NAME
    include.write_text(render_include(cfg=cfg, identity=identity, wrapper=wrapper), encoding="utf-8")
    user_cfg = ensure_user_include(ssh_dir)
    log(f"SSH include → {include}")
    log(f"ключ → {identity}")
    log(f"pubkey → {pub}")
    return {"include": include, "config": user_cfg, "identity": identity, "pub": pub, "wrapper": wrapper}
