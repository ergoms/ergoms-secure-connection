...
def _load_state(var_dir: Path) -> dict[str, Any]:
...
def _save_state(var_dir: Path, data: dict[str, Any]) -> None:
...
def _update_state(var_dir: Path, **fields: object) -> dict[str, Any]:
...
def _gateway_win(dest: str) -> str | None:
    ...
    if dest:
        try:
            from desktop.tun import iface_ipv4s, win_if_alias

            idx = _iface_index_win(dest)
            name = win_if_alias(idx) if idx else None
...
def _win_loopback_index() -> int:
    try:
        from desktop.tun import win_if_index_by_alias
    except Exception:
        return 1
    for name in ("Loopback Pseudo-Interface 1", "Псевдоинтерфейс петли 1"):
        idx = win_if_index_by_alias(name)
        if idx:
            return int(idx)
    return 1
...
def prefer_tun_ipv4(tun_idx: int, *, var_dir: Path, log: LogFn = noop) -> None:
    ...
    if sys.platform != "win32" or not tun_idx:
        return
    from desktop.tun import is_virtual_underlay, underlay_ifaces
