...
def _build(
    *,
    ...
    with (
        patch("desktop.singbox_mode.resolve_host", return_value="193.23.202.147"),
        ...
        return mgr.build_config(
            ...
            corporate_proxy="10.16.0.8:3128" if office else "",
            socks_port=1080,
            http_port=1088,
...
def test_build_config_office_vless_via_squid() -> None:
    ...
    assert tags["squid"]["server"] == "10.16.0.8"
...
def test_win_kill_switch_install_pins_and_blackholes() -> None:
    ...
    assert "route add 193.23.202.147 mask 255.255.255.255 10.193.0.1 metric 1" in joined
    assert "netsh interface ipv6 add route ::/1" in joined


def test_linux_kill_switch_install_blackhole() -> None:
    cmds = _cmds_linux_install(["193.23.202.147"], "10.193.0.1", blackhole=True)
    assert "ip route replace 0.0.0.0/1 dev lo metric 512" in cmds
    assert "ip route replace 193.23.202.147/32 via 10.193.0.1" in cmds
...
def test_build_pac_full_and_github() -> None:
    ...
    assert "PROXY 10.16.0.8:3128" in github
